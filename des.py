...
def encrypt(name):
    ...
    string = input()
    plaintext = str2plaintext(string)
    plaintext_int = binstr2int(plaintext)
    plaintext_hex = hex(plaintext_int)  # hex得到的是字符串，在int2binlist时出错
    print("待加密的十六级进制:{}".format(plaintext_hex))
    print("待加密二进制串:{}".format(plaintext))
    ciphertext = hex(Des(plaintext_int, key, "encrypt"))  # 传入的plaintext是64位0，1，64
    print("the encrypted ciphertext is {}".format(ciphertext))

def decrypt(name):
    print("input the word you want to decrypt:")
    string = input()
    string2int = int(string, 16)  # 以16进制串的形式看待string，并转为int整数
    ...
    print("the decrypted plain text is {}".format(plaintext_dec))
